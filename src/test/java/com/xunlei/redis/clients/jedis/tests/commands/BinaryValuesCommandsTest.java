...
import static com.xunlei.redis.clients.jedis.tests.utils.AssertUtil.assertByteArrayListEquals;
...
import com.xunlei.redis.clients.jedis.Protocol.Keyword;
...
public class BinaryValuesCommandsTest extends JedisCommandTestBase {
  ...
  public void setNxExAndGet() {
    String status = jedis.set(bfoo, binaryValue, SetParams.setParams().nx().ex(expireSeconds));
    assertTrue(Keyword.OK.name().equalsIgnoreCase(status));
    byte[] value = jedis.get(bfoo);
    assertTrue(Arrays.equals(binaryValue, value));
  ...
  public void setIfNotExistAndGet() {
    ...
    String statusFail = jedis.set(bfoo, binaryValue, SetParams.setParams().nx().ex(expireSeconds));
  ...
  public void setIfExistAndGet() {
    ...
    assertTrue(Keyword.OK.name().equalsIgnoreCase(status));
    // nx should fail if value exists
    String statusSuccess = jedis.set(bfoo, binaryValue, SetParams.setParams().xx().ex(expireSeconds));
  ...
  public void setFailIfNotExistAndGet() {
    ...
    String statusFail = jedis.set(bfoo, binaryValue, SetParams.setParams().xx().ex(expireSeconds));
    assertNull(statusFail);
  }

  @Test
  public void setAndExpireMillis() {
    String status = jedis.set(bfoo, binaryValue, SetParams.setParams().nx().px(expireMillis));
    assertTrue(Keyword.OK.name().equalsIgnoreCase(status));
