...
import com.xunlei.redis.clients.jedis.commands.RedisPipeline;
...
import com.xunlei.redis.clients.jedis.BinaryClient.LIST_POSITION;
import com.xunlei.redis.clients.jedis.commands.BinaryRedisPipeline;
...
public abstract class PipelineBase extends Queable implements BinaryRedisPipeline, RedisPipeline {
  ...
  public Response<Long> zcount(byte[] key, double min, double max) {
    getClient(key).zcount(key, Protocol.toByteArray(min), Protocol.toByteArray(max));
    return getResponse(BuilderFactory.LONG);
  }
  ...
  public Response<Set<Tuple>> zrangeByScoreWithScores(byte[] key, double min, double max,
      int offset, int count) {
    getClient(key).zrangeByScoreWithScores(key, Protocol.toByteArray(min), Protocol.toByteArray(max), offset, count);
    return getResponse(BuilderFactory.TUPLE_ZSET_BINARY);
  }
  ...
  public Response<Set<byte[]>> zrevrangeByScore(byte[] key, double max, double min) {
    getClient(key).zrevrangeByScore(key, Protocol.toByteArray(max), Protocol.toByteArray(min));
    return getResponse(BuilderFactory.BYTE_ARRAY_ZSET);
  }
  ...
  public Response<Set<byte[]>> zrevrangeByScore(byte[] key, double max, double min, int offset,
      int count) {
    getClient(key).zrevrangeByScore(key, Protocol.toByteArray(max), Protocol.toByteArray(min), offset, count);
    return getResponse(BuilderFactory.BYTE_ARRAY_ZSET);
  }
  ...
  public Response<Set<Tuple>> zrevrangeByScoreWithScores(byte[] key, double max, double min) {
    getClient(key).zrevrangeByScoreWithScores(key, Protocol.toByteArray(max), Protocol.toByteArray(min));
    return getResponse(BuilderFactory.TUPLE_ZSET_BINARY);
  }
  ...
  public Response<Set<Tuple>> zrevrangeByScoreWithScores(byte[] key, double max, double min,
      int offset, int count) {
    getClient(key).zrevrangeByScoreWithScores(key, Protocol.toByteArray(max), Protocol.toByteArray(min), offset,
      count);
    return getResponse(BuilderFactory.TUPLE_ZSET_BINARY);
  }
  ...
  public Response<Long> zremrangeByScore(byte[] key, double start, double end) {
    getClient(key).zremrangeByScore(key, Protocol.toByteArray(start), Protocol.toByteArray(end));
    return getResponse(BuilderFactory.LONG);
  }
